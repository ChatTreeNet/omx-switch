...
import type { CategoryConfig, OhMyOpenAgentConfig } from '@/types/omoConfig';
...
const configWithFutureFields: OhMyOpenAgentConfig = {
...
describe('omoConfig', () => {
  ...
  describe('config echo bug fixes', () => {
    ...
    it('normalizes missing openEditorTargetMode to remote', () => {
      expect(normalizeOmoGuiConfig({ stickyBusyDelayMs: 1000 })).toEqual({
    ...
    it('preserves hub openEditorTargetMode when configured', () => {
      expect(normalizeOmoGuiConfig({ openEditorTargetMode: 'hub' })).toEqual({
    ...
    it('injects the canonical schema when writing the default config path', async () => {
      ...
      try {
        const { CONFIG_PATH: defaultConfigPath, readConfig: readDefaultConfig, writeConfig: writeDefaultConfig } = await import('./omoConfig');
    ...
    it('type-checks rich v4 fallback models and future config/category fields', () => {
      expect(typedV4Config.agents?.sisyphus?.fallback_models).toEqual([
        'openai/gpt-5.4',
        expect.objectContaining({ reasoningEffort: 'max', maxTokens: 32000 }),
      ]);
      expect(configWithFutureFields.categories?.ultrabrain.futureCategoryField).toBe(true);
      expect(configWithFutureFields.futureConfigField).toEqual({ enabled: true });
    });
  });
});

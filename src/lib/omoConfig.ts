...
import type { OhMyOpenAgentConfig, OpenEditorTargetMode, OmoGuiConfig } from '@/types/omoConfig';
...
export type OmoConfig = OhMyOpenAgentConfig;
...
export interface LegacyMigrationOptions {
  ...
  backupPath?: string;
  now?: () => Date;
  writeCanonicalConfig?: (config: OmoConfig, canonicalPath: string) => Promise<void>;
}
...
export function normalizeOmoGuiConfig(value: unknown): OmoGuiConfig {
...
export function mergeConfig(base: OmoConfig, overlay: OmoConfig): OmoConfig {
  ...
  for (const [key, overlayValue] of Object.entries(overlay)) {
    ...
    if (isPlainObject(baseValue) && isPlainObject(overlayValue)) {
      merged[key] = mergeConfig(baseValue as OmoConfig, overlayValue as OmoConfig);
      continue;
    }
  ...
  return merged as OmoConfig;
}

export async function readConfig(configPath: string = CONFIG_PATH): Promise<OmoConfig> {
  try {
    const content = await readFile(resolveConfigPath(configPath), 'utf-8');
    const config = parse(content, null, false) as OmoConfig;
    return config;
  } catch {
    return {};
  }
...
async function readLegacyConfigForMigration(legacyPath: string): Promise<OmoConfig> {
  try {
    const content = await readFile(legacyPath, 'utf-8');
    return parse(content, null, false) as OmoConfig;
  } catch (error) {
    throw new Error(`Failed to read legacy config: ${error}`);
  }
}

export async function readEffectiveConfig(options: EffectiveConfigOptions = {}): Promise<OmoConfig> {
...
export async function writeConfig(
  ...
  try {
    ...
    const configWithSchema: OmoConfig = shouldEnforceSchema
      ? {
          ...config,
